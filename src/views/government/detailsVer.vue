<template>
  <div class="detailsVer">
    ...
    <div class="messBox">
      <div class="headbox">
        <img :src="obj.content_pic | formatImg" alt srcset />
      </div>
      <div class="txt">
        <span>{{ obj.CreateName || obj.CreateUserName }}</span>
    ...
    <div class="activity">
      ...
      <button class="btnB" @click="VSignIn" v-if="up(obj.start_time)">点击报名</button>
    </div>
    <!-- 内容 -->
...
export default {
  ...
  methods: {
    VSignIn() {
      //当前活动id
      const activity_id = this.obj.activity_id;
  ...
  computed: {
    up(start) {
      return function(start) {
        var thisTime = start.replace(/-/g, "/"); //转换为时间戳
