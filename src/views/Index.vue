<template>
  <div id="Index">
    ...
    <Popup v-model="show" position="bottom" :style="{ height: '55%' }" class="Popup">
      <div class="title">所在区域</div>
      <!--显示地址-->
      <div class="address">
        <div
          :class="{active:addressLevel === 0}"
          @click="getAddress(0)"
        >{{province.areaName? province.areaName : "省"}}</div>
        <div
          :class="{active:addressLevel === 1}"
          @click="getAddress(1)"
        >{{city.areaName ? city.areaName : "市"}}</div>
        <div
          :class="{active:addressLevel === 2}"
          @click="getAddress(2)"
        >{{district.areaName ? district.areaName : "区/县"}}</div>
        <div
          :class="{active:addressLevel === 3}"
          @click="getAddress(3)"
        >{{street.areaName ? street.areaName : "街道/镇"}}</div>
        <div
          :class="{active:addressLevel === 4}"
          @click="getAddress(4)"
        >{{community.areaName ? community.areaName : "社区/村"}}</div>
      </div>
      <Picker
        :columns="columns"
        @change="onChange"
...
export default {
  ...
  data() {
    return {
      toolsList: [
        ...
        {
          title: "人才兴村",
          img: baseImgUrl + "rencaixincun.png",
          pageRouter: "/Person.html"
        },
        {
          title: "产业富村",
          img: baseImgUrl + "wenminglecun.png",
          pageRouter: "/estate.html"
        },

        {
          title: "文明乐村",
          img: baseImgUrl + "changyefucun.png",
          pageRouter: "/Civilization.html"
        },
        {
          title: "更多服务",
  ...
  methods: {
    ...
    getSwiper() {
      ...
      let deptId = userInfo === null ? 24156 : userInfo.deptId;
      getSwiper({
    ...
    //获取党/村/政务
    getIndexNewsClass(news_type = 215) {
      //显示弹出层
